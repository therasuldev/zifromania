...
final appRouterProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      if (authState.isLoading) return isSplash ? null : RouteNames.splash;
      if (authState.value != null && isLogin) return RouteNames.home;
      if (authState.value == null && !isSplash && !isLogin) return RouteNames.login;
      return null;
    },
    routes: [
      GoRoute(path: RouteNames.splash, builder: (context, state) => const VersionarteGate(child: SplashScreen())),
      GoRoute(path: RouteNames.login, builder: (context, state) => const AuthScreen()),
      GoRoute(path: RouteNames.home, builder: (context, state) => const GameIntroScreen()),
      GoRoute(
        path: '${RouteNames.game}/:category',
        pageBuilder: (context, state) {
          final category = _categoryFromName(state.pathParameters['category']);
          if (category == null) return _errorPage('Invalid game category');
          return _modalPage(
            state,
            GameScreen(
      ...
      GoRoute(
        ...
        pageBuilder: (context, state) => _modalPage(
          state,
          SubscriptionScreen(tabType: _tabFromName(state.uri.queryParameters['tab'])),
        ),
      ),
      GoRoute(path: RouteNames.leaderboard, pageBuilder: (context, state) => _modalPage(state, const LeaderboardScreen())),
      GoRoute(path: RouteNames.achievements, pageBuilder: (context, state) => _modalPage(state, const AchievementsScreen())),
      GoRoute(path: RouteNames.settings, pageBuilder: (context, state) => _modalPage(state, const SettingsScreen())),
      GoRoute(path: RouteNames.feedback, pageBuilder: (context, state) => _modalPage(state, const FeedbackScreen())),
      GoRoute(path: RouteNames.about, pageBuilder: (context, state) => _modalPage(state, const AboutZifroManiaScreen())),
      GoRoute(path: RouteNames.privacy, pageBuilder: (context, state) => _modalPage(state, const PrivacyPolicyScreen())),
      GoRoute(path: RouteNames.terms, pageBuilder: (context, state) => _modalPage(state, const TermsOfServiceScreen())),
    ],
  );
});
...
class _RouterRefreshNotifier extends ChangeNotifier {
  _RouterRefreshNotifier(Ref ref) {
    ref.listen(authNotifierProvider, (previous, next) {
      if (previous?.value != next.value || previous?.isLoading != next.isLoading) notifyListeners();
    });
  }
}
...
TabType _tabFromName(String? name) => name == 'coins' ? TabType.coins : TabType.subscription;

Page<void> _modalPage(GoRouterState state, Widget child) => CustomTransitionPage<void>(
      key: state.pageKey,
      opaque: false,
      ...
      child: Align(
        ...
        child: Material(
          ...
          child: FractionallySizedBox(heightFactor: .99, widthFactor: 1, child: child),
        ),
      ),
...
Page<void> _errorPage(String message) => MaterialPage<void>(
